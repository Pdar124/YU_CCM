...
function Signup({ onSignupSuccess }) {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      const emailFormat =
        signupType === 'student'
          ? `${studentId.trim()}${SCHOOL_DOMAIN}`
          : `${phone.trim()}${SCHOOL_DOMAIN}`;
      ...
      await setDoc(doc(db, 'users', user.uid), {
        uid: user.uid,
        studentId: signupType === 'student' ? studentId.trim() : '',
        phone: signupType === 'phone' ? phone.trim() : '',
        email: emailFormat,
        nickname: accountId,
        role: 'student',
        signupType,
            ...
            <div key={num} className="flex-1 flex flex-col items-center relative">
              ...
              <span
                className={`mt-2 text-xs font-semibold ${
                  num === '1' ? 'text-emerald-600' : 'text-slate-500'
                }`}
              >
                {label}
              </span>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`p-4 rounded-2xl border flex items-center justify-center gap-2 font-bold ${
                  signupType === 'student'
