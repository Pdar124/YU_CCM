...
<img width="4096" height="1581" alt="Image" src="https://github.com/user-attachments/assets/683cfedd-bdac-4754-90e4-937d8459683a" />

Figure 2.1은 CCM 시스템의 Component, custom hook, service module, Firestore data model 간의 관계를 나타낸다. Component는 사용자 입력과 화면 상태를 제어하고, custom hook은 반복되는 데이터 구독과 계산 로직을 분리한다. service module은 Firebase 및 외부 API 접근을 담당하며, Firestore data model은 시스템에서 관리하는 핵심 Entity를 표현한다.

본 다이어그램을 기준으로 화면 계층, 데이터 구독 및 계산 계층, 외부 데이터 접근 계층, 저장 데이터 구조를 분리하여 구현할 수 있다.

## 2.2 Class Description

Firestore data model은 시스템에서 관리하는 핵심 Entity이며, 각 Entity는 주요 속성과 해당 속성에 접근하거나 값을 변경하는 getter/setter operation을 가진다. Class Description에서는 Component, custom hook, service module, Firestore data model의 책임과 주요 operation을 정리한다. Getter와 setter operation은 데이터 보관과 상태 변경 책임을 가지는 Entity 클래스에 표시하고, Component, custom hook, service module은 화면 제어, 데이터 구독, 외부 데이터 접근에 필요한 주요 operation만 표시하였다.
...
| LoginPage | 사용자의 로그인 입력을 받고 Firebase Authentication 인증 요청을 시작한다. | handleLoginSubmit(): 학번 또는 계정 정보를 인증 형식으로 변환하고 로그인 요청을 수행한다. |
| SignupPage | 신규 사용자의 회원가입 입력과 계정 생성을 처리한다. | handleSignupSubmit(): 회원가입 정보를 검증하고 사용자 계정 생성 흐름을 시작한다. |
...
| MapContainer | Kakao Map을 렌더링하고 지도 객체를 화면 컴포넌트와 연결한다. | MapContainer(): 지도 초기화 상태와 ref를 받아 지도 영역을 표시한다. |
...
| Data Model | Responsibility | Main Fields | Getter / Setter Operations |
| --- | --- | --- | --- |
| User | 사용자 기본 정보와 권한 상태를 저장한다. | uid, studentId, phone, email, nickname, department, role, activeMode, caregiverCatIds, createdAt | getUid(), getStudentId(), getPhone(), getEmail(), getNickname(), getDepartment(), getRole(), getActiveMode(), getCaregiverCatIds(), getCreatedAt(), setPhone(), setNickname(), setDepartment(), setRole(), setActiveMode(), setCaregiverCatIds() |
| Cat | 고양이 프로필, 위키 정보, 위치, 상태를 저장한다. | id, name, gender, imageUrl, description, origin, feature, healthStatus, territory, estimatedAge, weight, lat, lng, location, status | getId(), getName(), getGender(), getImageUrl(), getDescription(), getOrigin(), getFeature(), getHealthStatus(), getTerritory(), getEstimatedAge(), getWeight(), getLat(), getLng(), getLocation(), getStatus(), setName(), setGender(), setImageUrl(), setDescription(), setOrigin(), setFeature(), setHealthStatus(), setTerritory(), setEstimatedAge(), setWeight(), setLat(), setLng(), setLocation(), setStatus() |
| Report | 고양이 목격 제보를 저장한다. | id, catId, lat, lng, memo, imageUrl, reporterUid, reporterName, observedAt, createdAt | getId(), getCatId(), getLat(), getLng(), getMemo(), getImageUrl(), getReporterUid(), getReporterName(), getObservedAt(), getCreatedAt(), setCatId(), setLat(), setLng(), setMemo(), setImageUrl(), setReporterUid(), setReporterName(), setObservedAt() |
| DietLog | 돌보미의 급여 및 건강 기록을 저장한다. | id, catId, catName, caregiverUid, caregiverName, foodType, amount, symptoms, memo, fedAt | getId(), getCatId(), getCatName(), getCaregiverUid(), getCaregiverName(), getFoodType(), getAmount(), getSymptoms(), getMemo(), getFedAt(), setFoodType(), setAmount(), setSymptoms(), setMemo() |
| WikiHistory | 고양이 위키 수정 이력을 저장한다. | id, catId, editorUid, editorName, origin, feature, healthStatus, territory, estimatedAge, weight, editedAt | getId(), getCatId(), getEditorUid(), getEditorName(), getOrigin(), getFeature(), getHealthStatus(), getTerritory(), getEstimatedAge(), getWeight(), getEditedAt(), setOrigin(), setFeature(), setHealthStatus(), setTerritory(), setEstimatedAge(), setWeight() |
| CaregiverRequest | 돌보미 신청과 승인 상태를 저장한다. | id, uid, studentId, nickname, catIds, reason, authCode, status, createdAt, approvedAt, rejectedAt | getId(), getUid(), getStudentId(), getNickname(), getCatIds(), getReason(), getAuthCode(), getStatus(), getCreatedAt(), getApprovedAt(), getRejectedAt(), setCatIds(), setReason(), setAuthCode(), setStatus() |
| CatRegistrationRequest | 신규 고양이 등록 요청과 승인 상태를 저장한다. | id, tempName, gender, description, requesterUid, requesterName, lat, lng, observedAt, status, createdAt, approvedAt, rejectedAt | getId(), getTempName(), getGender(), getDescription(), getRequesterUid(), getRequesterName(), getLat(), getLng(), getObservedAt(), getStatus(), getCreatedAt(), getApprovedAt(), getRejectedAt(), setTempName(), setGender(), setDescription(), setLat(), setLng(), setObservedAt(), setStatus() |
| Shelter | 쉼터 위치 정보를 저장한다. | id, name, lat, lng, location, description | getId(), getName(), getLat(), getLng(), getLocation(), getDescription(), setName(), setLat(), setLng(), setLocation(), setDescription() |
| Weather | OpenWeather 응답을 화면 표시용으로 변환한 날씨 데이터이다. | main, description, temperature, isRain | getMain(), getDescription(), getTemperature(), getIsRain(), setMain(), setDescription(), setTemperature(), setIsRain() |

Entity의 getter operation은 화면 컴포넌트와 service module이 필요한 속성 값을 조회할 때 사용되는 접근 책임을 의미한다. setter operation은 사용자의 입력, 관리자 승인, 위키 수정, 급여 기록 작성 등으로 인해 Entity의 상태가 변경될 때 필요한 수정 책임을 의미한다. 반면 Component, custom hook, service module은 데이터를 직접 보관하는 Entity가 아니므로 getter/setter 대신 각 계층의 역할에 맞는 이벤트 처리, 데이터 구독, 생성·수정 operation을 중심으로 표현하였다.
