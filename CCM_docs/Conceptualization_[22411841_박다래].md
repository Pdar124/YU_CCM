...
| **Approach** | 사용자가 '제보하기' 버튼을 누르면 사진 URL, 발견 시간, GPS 좌표를 제보 기록 자료로 저장한다. 시스템은 클릭한 좌표와 고양이의 활동 좌표, 최근 제보 기반 AI 예측 위치, 과거 제보 시간대를 비교하여 영역 매칭도가 높은 후보 고양이를 자동 추천(Territory-based ID)한다. |
...
| **<font color="blue">Path Analysis</font>** | **[Problem]** 단순 점 제보만으로는 이동 흐름 파악이 어려워 마주치기 힘듦. <br>**[Solution]** 최근 제보 3건을 제보자가 입력한 발견 시간순으로 연결하여 이동 흐름을 시각화한다. |
